...
import { useProviderAlerts } from '@features/provider/hooks';
import type { ProviderAlert } from '@features/provider/types';
...
export function ProviderAlertsBell() {
  const { alerts, isLoading } = useProviderAlerts();
  ...
  const panel = open && typeof document !== 'undefined' && (
    <div
      ...
      role="dialog"
      aria-label="Alertas del panel proveedor"
    >
      <div className="border-b border-border px-3 py-2">
        <h2 className="text-sm font-medium text-foreground">Alertas del panel</h2>
        <p className="text-xs text-muted-foreground">
          Planes, empresas, Stripe
        </p>
      </div>
      <div className="max-h-[min(70vh,400px)] overflow-y-auto overflow-x-hidden p-2">
        {sorted.length === 0 ? (
          ...
          <ul className="space-y-1">
            {sorted.map((alert: ProviderAlert) => (
              <li key={alert.code}>
                <Link
  ...
  return (
    <div className="relative flex items-center">
      <button
        ...
        aria-label={alerts.length > 0 ? `${alerts.length} alertas del panel` : 'Alertas del panel'}
        aria-expanded={open}
      >
