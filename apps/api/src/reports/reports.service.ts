...
export class ReportsService {
    ...
    try {
      ...
      if (e && typeof e === 'object' && 'code' in e) {
        ...
        if (code === 'P2021' || code === 'P2022') {
          this.logger.warn(`Report failed (Prisma ${code}): ${SCHEMA_ERROR_MESSAGE}`);
  ...
  async getOperationalState(): Promise<OperationalStateResponse> {
    ...
    invoicesDueSoonEnd.setDate(invoicesDueSoonEnd.getDate() + INVOICES_DUE_SOON_DAYS);
    ...
    if (todaySales.length === 0) {
      alerts.push({
        ...
        message: 'No se han registrado ventas pagadas hoy. Revisa caja o registra ventas.',
        area: 'sales',
        count: 0,
  ...
  async getActionableIndicators(
    ...
    if (overdueInvoices.length > 0) {
      ...
      const pendingTotal = overdueInvoices.reduce(
        (acc, inv) =>
          acc + Number(inv.grandTotal) - Number(inv.paidAmount),
        0,
      );
      indicators.push({
        code: 'INVOICES_OVERDUE',
    ...
    const marginBelowAvgProducts = allProducts.filter((p) => {
      ...
      return avgMargin > 0 && marginPct < avgMargin * MARGIN_BELOW_AVG_THRESHOLD;
    });

    if (marginBelowAvgProducts.length > 0 && avgMargin > 0) {
      const topBelowAvg = marginBelowAvgProducts
        .slice(0, top)
        .map(
          (p): ActionableIndicatorItem => ({
            id: p.id,
      ...
      indicators.push({
        ...
        severity: 'low',
        suggestedAction: 'Revisar precio o costo; alinear con la media del catálogo.',
        actionLabel: 'Ver productos',
        actionHref: '/products',
        items: topBelowAvg,
        detectedAt,
    ...
    reorderPeriodStart.setDate(reorderPeriodStart.getDate() - reorderDaysWindow);
    ...
    const reorderCandidates: { productId: string; daysOfStock: number; qtyOnHand: number }[] = [];
    ...
    if (reorderCandidates.length > 0) {
      ...
      const reorderProductIds = reorderCandidates.slice(0, top).map((c) => c.productId);
      ...
      const reorderItems = reorderCandidates
        .slice(0, top)
        .map(
      ...
      indicators.push({
        ...
        severity: 'medium',
        suggestedAction: 'Evaluar pedido a proveedor para evitar quiebre de stock.',
    ...
    const todayTotal = salesToday.reduce((acc, s) => acc + Number(s.grandTotal), 0);
    ...
    if (avgDaily7 > 0 && todayTotal < avgDaily7 * ANOMALY_THRESHOLD) {
      indicators.push({
        ...
        severity: 'medium',
        suggestedAction: 'Revisar causas (horario, disponibilidad, promociones) o considerar campaña.',
    ...
    const sortedDays = Array.from(totalByDay.entries())
    ...
    if (sortedDays.length > 0) {
      ...
      if (avgDailySales <= 0) {
        ...
        avgDailySales = sortedDays.length > 0 ? sumPeriod / sortedDays.length : 0;
      }
    }
    if (avgDailySales > 0) {
      ...
      indicators.push({
        ...
        severity: 'info',
        suggestedAction: 'Usar como referencia para pedidos a proveedor y planificación de caja.',
        actionLabel: 'Ver ventas',
        actionHref: '/sales',
        items: [
          { id: '7d', name: 'Próximos 7 días', value: `~$${forecast7.toLocaleString()}` },
          { id: '30d', name: 'Próximos 30 días', value: `~$${forecast30.toLocaleString()}` },
        ],
        detectedAt,
      });
    }
    ...
    for (const [id, v] of sortedByAmount) {
      if (v.lastSale < sixtyDaysAgo) {
        inactivos++;
        if (segmentItems.length < top && segmentItems.every((i) => i.id !== id))
          segmentItems.push({
            id,
            name: v.name,
    ...
    if (customerStatsMap.size > 0) {
      indicators.push({
        ...
        severity: 'info',
        suggestedAction: 'Reactivar inactivos con ofertas; priorizar servicio a premium.',
    ...
    const supplierScores = new Map<
      string,
      { name: string; priceAboveAvg: boolean; onTimePct: number; overdueCount: number }
    >();
    const supplierIdsFromPo = new Set(posWithSupplier.map((p) => p.supplierId));
    ...
    if (scoreList.length > 0) {
      indicators.push({
        ...
        severity: 'info',
        suggestedAction: 'Priorizar proveedores con score alto; renegociar o revisar los de score bajo.',
        actionLabel: 'Ver proveedores',
        actionHref: '/suppliers',
  ...
  async getCustomerClusters(dto: CustomerClustersDto = {}): Promise<{
    ...
    const daysAgo = entries.map(([, v]) =>
      (now.getTime() - v.lastSale.getTime()) / (1000 * 60 * 60 * 24),
    );
    ...
    const clusterIndices = result.clusters as number[];
    ...
    const clustersOut = clustersRaw
      ...
      .map((cluster, rank) => {
        ...
        const description = descriptionsByRank[rank] ?? `Segmento por comportamiento de compra (${cluster.customers.length} clientes).`;
        return {
  ...
  async exportAsCsv(
    ...
    const entity = dto.entity as ExportEntity;
    ...
    throw new BadRequestException(`Entidad no soportada para export: ${entity}`);
  }
}
