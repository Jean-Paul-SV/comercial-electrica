...
- **Fase 2 (anomalías simples):** Desviación respecto a la media o al percentil (ventas del día vs media de últimos 7 días; margen de producto vs media). Sin modelo entrenado; solo estadística descriptiva.  
- **Fase 3 (IA):** Modelos de pronóstico (ARIMA, Prophet o modelo simple por producto), clustering (K-means en clientes), o resumen con LLM (una frase del dashboard). Preferir servicios gestionados (ej. pronóstico en BigQuery, LLM vía API) si el equipo no tiene expertise en ML.
